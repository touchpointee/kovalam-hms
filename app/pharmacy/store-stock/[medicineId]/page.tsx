...
export default function MedicineBatchPage() {
  ...
  const loadBatches = useCallback(async () => {
    ...
    params.set("inventoryType", "store");
      ...
      <div className="rounded-xl border border-emerald-100 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="space-y-2">
            <Link href={`${pharmacyBase}/store-stock`} className="inline-flex items-center gap-2 text-sm text-teal-700 hover:underline">
              <ArrowLeft className="h-4 w-4" />
              Back to store stock
            </Link>
            <div>
              <h1 className="text-3xl font-semibold text-slate-800">{medicine?.name ?? "Store Batches"}</h1>
              <p className="text-sm text-slate-500">
                {medicine?.genericName || medicine?.manufacturer || "Manage store batches and stock transactions for this medicine."}
              </p>
