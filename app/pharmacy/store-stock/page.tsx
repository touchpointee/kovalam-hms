...
export default function PharmacyStockPage() {
    ...
    try {
      ...
      const stockRows = await Promise.all(
        rows.map(async (medicine) => {
          const params = new URLSearchParams({ medicineId: medicine._id, inventoryType: "store" });
  ...
  const goBatches = (medicineId: string) => {
    router.push(`${pharmacyBase}/store-stock/${medicineId}`);
  };
    ...
    <div className="space-y-5">
      <div className="rounded-xl border border-emerald-100 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-3xl font-semibold text-slate-800">Store Stock Management</h1>
            <p className="text-sm text-slate-500">
              Manage the central medicine catalog and store inventory before transferring stock to pharmacy.
            </p>
          </div>
        </div>
