...
export const PATCH = withRouteLog("billingStaff.id.PATCH", async (
  req: NextRequest,
  ...
  try {
    ...
    const updated = await BillingStaff.findByIdAndUpdate(id, { $set: update }, { new: true }).lean();
    if (!updated) {
      return NextResponse.json({ message: "Not found" }, { status: 404 });
    }
    return NextResponse.json({
      ...updated,
      label: updated.code?.trim() ? `${updated.name} ${updated.code.trim()}` : updated.name,
    });
  } catch (e) {
...
export const DELETE = withRouteLog("billingStaff.id.DELETE", async (
  _req: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) => {
  try {
    ...
    const updated = await BillingStaff.findByIdAndUpdate(
      id,
      { $set: { isActive: false } },
      { new: true }
    ).lean();
