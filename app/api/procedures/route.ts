...
export const GET = withRouteLog("procedures.GET", async (req: NextRequest) => {
  try {
    ...
    const procedures = await Procedure.find(filter).sort({ name: 1 }).lean();
...
export const POST = withRouteLog("procedures.POST", async (req: NextRequest) => {
  try {
    ...
    const procedure = await Procedure.create(parsed.data);
    return NextResponse.json(procedure.toJSON());
  } catch (e) {
    console.error(e);
    return NextResponse.json(
      { message: e instanceof Error ? e.message : "Server error" },
