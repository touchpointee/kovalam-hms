...
export const GET = withRouteLog("medicineGroups.GET", async (req: NextRequest) => {
  try {
    await dbConnect();
    ...
    const rows = await MedicineGroup.find(filter).sort({ name: 1 }).lean();
...
export const POST = withRouteLog("medicineGroups.POST", async (req: NextRequest) => {
  try {
    await dbConnect();
    ...
    const existing = await MedicineGroup.findOne({
    ...
    const row = await MedicineGroup.create({ name });
